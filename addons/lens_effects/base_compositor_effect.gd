...
# Original code can be found here: https://github.com/pink-arcana/godot-distance-field-outlines/
# Small changes made by ARez
...
const COLOR_SAMPLER_BINDING : int = 4
...
func _initialize_render_base() -> void:
	...
	linear_sampler = create_sampler(RenderingDevice.SamplerFilter.SAMPLER_FILTER_LINEAR, RenderingDevice.SamplerRepeatMode.SAMPLER_REPEAT_MODE_CLAMP_TO_EDGE)
...
func create_uniform_buffer(p_data : Array) -> RID:
	var buffer_data := _get_buffer_data(p_data)
	var ubo : RID = rd.uniform_buffer_create(buffer_data.size(), buffer_data)
	add_rid_to_free(ubo, "ubo")
	return ubo


func update_uniform_buffer(buffer_rid: RID, p_data: Array) -> Error:
	var buffer_data := _get_buffer_data(p_data)
	var update_err := rd.buffer_update(buffer_rid, 0, buffer_data.size(), buffer_data)
	if update_err != 0:
...
func _get_buffer_data(p_data: Array) -> PackedByteArray:
	...
	for value in p_data:
		...
		match type:
			...
			_:
				push_error("[BaseCompositorEffect:create_uniform_buffer()] Unhandled data type found: %s" % type)
				continue
	...
	const resize_base := 16
	if buffer_data.size() % resize_base:
		var divisor := floori(float(buffer_data.size()) / float(resize_base))
		buffer_data.resize((divisor + 1) * resize_base)

	if print_buffer_resize and buffer_data.size() != size_before:
...
func get_color_image_uniform(p_view : int) -> RDUniform:
	# Should be DATA_FORMAT_R16G16B16A16_SFLOAT
...
func get_color_sampler_uniform(p_view : int, p_sampler: RID) -> RDUniform:
	# Should be DATA_FORMAT_R16G16B16A16_SFLOAT
	var color_image : RID = render_scene_buffers.get_color_layer(p_view)
	var color_sampler_uniform : RDUniform = get_sampler_uniform(
			color_image,
			p_sampler,
			COLOR_SAMPLER_BINDING,
		)
	return color_sampler_uniform
...
func get_normal_sampler_uniform(_p_view : int, p_sampler : RID) -> RDUniform:
...
func _render_view(_p_view : int) -> void:
	pass
