...
当前第 1、2、3 个槽位沿用原有段落场景，第 4 个槽位先指向独立的回声之境场景。第 2、3、4 个槽位按前一段完成状态解锁；回声之境通关后，第 4 个槽位切换为完整场景。

## 四个场景槽位

场景路径统一维护在 `MainMenu.gd` 的 `STAGE_SCENE_PATHS`，第四个槽位初始对应独立的“回声之境”场景，完整内容解锁后切换到完整场景。

前三个槽位按“初滞、回声、终章”处理，第四个槽位先进入“回声之境”，通关后解锁完整内容。空字符串代表占位场景，菜单会自动禁用进入按钮。
...
- 从主菜单进入回声之境场景时使用一次性 `menu_launch_pending` 标记。`Player` 会跳过旧的游戏内 `StartPage` 并自动开始；直接在编辑器运行关卡场景时，原有 `StartPage` 仍然保留。
