...
# 配置变量 - 直接处理所有 MeshInstance3D 节点
@export var layer: int = 2  # Specify which layer to add StaticBody3D to.
...
func create_static_bodies_for_meshes() -> void:
	...
	for mesh_instance in mesh_instances:
		...
		# 设置 collision_layer 使用一个位掩码
		static_body.collision_layer = 1 << (layer - 1)  # Correcting layer using bitwise shift (starting from 1)
