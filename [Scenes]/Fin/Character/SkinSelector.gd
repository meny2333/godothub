...
func _apply_language() -> void:
	...
	skin_subtitle.text = "启程之前" if is_chinese else "BEFORE THE JOURNEY"
	...
	current_caption.text = "此刻同行" if is_chinese else "WALKING BY YOUR SIDE"
	...
	bottom_hint.text = "轻触外面，回到世界" if is_chinese else "TOUCH OUTSIDE TO RETURN"
	...
	settings_intro.text = "让声音与画面，与你的节奏同行。改动即刻生效。" if is_chinese else "SOUND AND VISION, IN STEP WITH YOUR BEAT. CHANGES TAKE EFFECT AT ONCE."
